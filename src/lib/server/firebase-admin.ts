import * as admin from 'firebase-admin';

let db: admin.firestore.Firestore;
let auth: admin.auth.Auth;
let storage: admin.storage.Storage;
let initError: string | null = null;

if (typeof window === 'undefined') {
  try {
    // Check if there are already initialized apps
    if (!admin.apps.length) {
      const projectId = process.env.FIREBASE_PROJECT_ID || "elba7rawy-49a39";
      const clientEmail = process.env.FIREBASE_CLIENT_EMAIL || "firebase-adminsdk-fbsvc@elba7rawy-49a39.iam.gserviceaccount.com";
      let privateKey = process.env.FIREBASE_PRIVATE_KEY;

      if (privateKey) {
        // Handle both escaped and literal newlines, and potential quotes from Vercel env
        privateKey = privateKey.replace(/\\n/g, '\n').trim();
        if (privateKey.startsWith('"') && privateKey.endsWith('"')) {
          privateKey = privateKey.slice(1, -1);
        }
      }

      // Final fallback to hardcoded key if environment variable is totally missing
      if (!privateKey) {
        privateKey = `-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n`.replace(/\\n/g, '\n');
      }

      admin.initializeApp({
        credential: admin.credential.cert({
          projectId,
          clientEmail,
          privateKey,
        }),
        storageBucket: process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET || "elba7rawy-49a39.firebasestorage.app",
      });
    }

    db = admin.firestore();
    auth = admin.auth();
    storage = admin.storage();
  } catch (error: any) {
    console.error('Firebase Admin Init Error:', error);
    initError = error.message;
    
    // Safety fallback to prevent code from crashing, but still reporting error
    const dummyQuery: any = {
      get: async () => ({ docs: [], empty: true, size: 0 }),
      limit: function() { return this; },
      where: function() { return this; },
      orderBy: function() { return this; },
    };
    
    db = {
      collection: () => ({
        ...dummyQuery,
        doc: () => ({
          get: async () => ({ exists: false }),
          set: async () => {},
          update: async () => {},
          delete: async () => {},
        }),
        add: async () => ({ id: 'temp' }),
      }),
      batch: () => ({
        delete: () => {},
        commit: async () => {},
      }),
    } as any;
    auth = {} as any;
    storage = { bucket: () => ({ file: () => ({ save: async () => {}, makePublic: async () => {} }) }) } as any;
  }
}

export { db, auth, storage, initError };
