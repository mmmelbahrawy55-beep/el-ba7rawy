...
if (typeof window === 'undefined') {
  try {
    ...
    if (!admin.apps || admin.apps.length === 0) {
      ...
      if (privateKey) {
        // Robust Private Key cleaning for Vercel
        privateKey = privateKey.replace(/\\n/g, '\n');
        if (privateKey.startsWith('"') && privateKey.endsWith('"')) {
          privateKey = privateKey.substring(1, privateKey.length - 1);
        }
      } else {
        // Fallback hardcoded key (only if env is missing)
    ...
    // Improved dummy object with all necessary methods to prevent crashes
    const dummyQuery = {
    ...
    auth = {};
    storage = { bucket: () => ({ file: () => ({ save: async () => {}, makePublic: async () => {} }) }) };
  }
}
